...
на вопрос «похоже ли это на то, что задумано». Фасонная модель может пройти
все числовые проверки, будучи сплошным клиновидным бруском: габарит совпадёт
с заданным точно, а узнаваемости не будет никакой — её дают вырезы, сужение
в плане, наклон стенок и проёмы, и ни одно из этих свойств габаритом
не измеряется.
...
def set_view(sw, doc, up, normal):
    ...
    штатная изометрия показывает модель лежащей на боку.
